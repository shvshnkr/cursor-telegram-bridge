...
def help_message(default_mode: str) -> str:
    return (
        "*Команды*\n\n"
        "*Режимы:*\n"
        "- кнопка `/ask`, `/plan` или `/agent` — выбрать режим, затем текст\n"
        "- `/ask вопрос` — сразу в ask (аналогично plan/agent)\n"
        "- `/plan` — краткий итог, затем полный план отдельным сообщением (не файлом)\n"
        "- без префикса и без выбранного режима — `%s`\n\n"
